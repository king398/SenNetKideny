...
def main(cfg):
    ...
    for epoch in range(cfg['epochs']):
        ...
        dice_score, surface_dice, false_positive = validation_fn(
        ...
        if false_positive < best_false_positive:
            best_false_positive = false_positive
        accelerate.save(model_weights, f"{cfg['model_dir']}/model_best_false_positive.pth")
        accelerate.print(f"Saved Model With Best False Positive Score {best_false_positive}")
        if epoch == 3:
